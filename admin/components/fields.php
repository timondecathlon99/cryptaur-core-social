  ...
  <div class="columns_list" > 
	 <div class='mob_menu_list'>
	    <div id="sortable1" class='sortable2 connectedSortable '>
          <?
		    $columns_sql = $pdo->prepare("SHOW COLUMNS FROM ".$_GET['type']);
            $columns_sql->execute();
  ...
  <div class="fields_list" > 
	 <div class='mob_menu_list'>
	    <div id="sortable2" class='sortable2 '>
          <?
		    $fields_sql = $pdo->prepare("SELECT * FROM fields");
            $fields_sql->execute();
	        while($field = $fields_sql->fetch()){?>
			  <div class='section timBox' id='block_<?=$field['id']?>~<?=$_GET['type']?>'>
			    <div class='block_content timBox'> 
